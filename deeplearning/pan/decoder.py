...
class FPABlock(nn.Module):
    ...
    def forward(self, x):
        ...
        b1 = self.branch1(x)  # 自适应平均池化 11
        upscale_parameters = dict(
            mode=self.upscale_mode,
            align_corners=self.align_corners
        )
        b1 = F.interpolate(b1, size=(h, w), **upscale_parameters)  # up-sample

        mid = self.mid(x)  # 11
        x1 = self.down1(x)  # 77
        x2 = self.down2(x1)  # 55
        x3 = self.down3(x2)  # 33
        x3 = F.interpolate(x3, size=(h // 4, w // 4), **upscale_parameters)  # up-sample

        x2 = self.conv2(x2)  # 55
        x = x2 + x3
        x = F.interpolate(x, size=(h // 2, w // 2), **upscale_parameters)  # up-sample

        x1 = self.conv1(x1)  # 77
        x = x + x1
        x = F.interpolate(x, size=(h, w), **upscale_parameters)  # up-sample
...
class GAUBlock(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),    # 自适应平均池化 11
...
class PANDecoder(nn.Module):
    ...
    def forward(self, *features):
        SEResNeXtBottleneck = features[-1]
        x5 = self.fpa(SEResNeXtBottleneck)  # 1/32
